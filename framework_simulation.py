...
def gNB(env, mnPower, timeResolution=31.25, totalNetworkSlices=1, printLogs=False):
	...
	TTLRar = 5*numerologySlot # (minimum ra-responseWindow interval {s5, s10, s20, s40, s80, s160, s320, s640, s1280})
	RRCInterval = 5*numerologySlot #interval to listen to RRC Connection Requests
...
def device(env, blePower, mnPower, timeResolution=31.25, index=0, status='Requester', totalNetworkSlices=1, discoverBeforeSIB1=False, backOff=False, framework=False, maxSym=14, maxSC=3, getFreqFromGNB=False, printLogs=False):
	...
	backOffTimer = 0
	backOffTimeOut = True
	RelayFoundTime = 5*numerologySlot # (minimum ra-responseWindow interval {s5, s10, s20, s40, s80, s160, s320, s640, s1280}), ra-responseWindow (after relay found) = 5 slots (5*66.67us -> SCS 15 kHz)
	...
	stopPRACH = False
	ra_responseWindow = 5*numerologySlot # (minimum ra-responseWindow interval {s5, s10, s20, s40, s80, s160, s320, s640, s1280})
	...
	################################
	# random start time in the simulation for every device
	start_back_off = random.randint(0, 50)
	...
	if status == 'Requester':
		...
		if printLogs == True: print(env.now, 'us: Device (' + str(index) + '): Requirements via Relay: ' + msg)
	RA_RNTI = ''
	...
	while totalRegisteredDevices < totalDev:
		if status == 'Requester':
			...
			if stopPRACH == False and preambleSelected == False and len(commonDownlinkBW[0]) == 64:
				sib1arrived = True
				...
				for dlRRCIndex, dlRRCChannel in enumerate(downlinkRRC):	
					if dlRRCChannel != 'None' and dlRRCChannel.split('|')[0].split('=')[1] == TC_RNTIRequester and dlRRCChannel.split('|')[1].split('=')[1] == str(randomValueRequester):
						if resourcesAllocated == 0:
							...
							print(env.now, 'us: Registered', totalRegisteredDevices, 'of', totalDev)
		...
		if status == 'Relay':
			...
			if getFreqFromGNB == False and start == 0 or env.now - slotTimer >= listennigWindow:
				slotTimer = env.now
...
def main(totalDevices, framework, discoverBeforeSIB1, totalNetworkSlices, maxSym, maxSC, blePower, mnPower, getFreqFromGNB, printLogs):
	...
	random.seed(7) # seed used to replicate random numbers
	...
	for bandIndex in range(len(collisionListBand24)):
		collisionListBand24[bandIndex] = [None for i in range(32)]
	global availableFreq24BandListening; availableFreq24BandListening = [0 for i in range(32)]

	env = simpy.Environment()
	for i in range(totalDevices):
		env.process(device(env=env, blePower=blePower, mnPower=mnPower, timeResolution=timeResolution, index=i, status='Requester', totalNetworkSlices=totalNetworkSlices, discoverBeforeSIB1=discoverBeforeSIB1, backOff=True, framework=framework, maxSym=maxSym, maxSC=maxSC, getFreqFromGNB=getFreqFromGNB, printLogs=printLogs))
...
if __name__ == '__main__':
	...
	# for test
	#histogram, logs = main(totalDevices, framework, discoverBeforeSIB1, 1, maxSym, maxSC, blePower, mnPower, getFreqFromGNB, printLogs)
	#print(logs)
	#print(histogram)
	...
	for totalD in [100, 200, 300, 400, 500, 600, 700]:
		histogram, logs = main(totalD, framework=False, discoverBeforeSIB1=False, totalNetworkSlices=totalNetworkSlices, maxSym=maxSym, maxSC=maxSC, blePower=blePower, mnPower=mnPower, getFreqFromGNB=False, printLogs=False)
		file = open('logs/01.log', 'a')
		file.write(logs + '\n')
	...
	for totalD in [100, 200, 300, 400, 500, 600, 700]:
		histogram, logs = main(totalD, framework=True, discoverBeforeSIB1=False, totalNetworkSlices=totalNetworkSlices, maxSym=maxSym, maxSC=maxSC, blePower=blePower, mnPower=mnPower, getFreqFromGNB=False, printLogs=False)
		file = open('logs/02.log', 'a')
		file.write(logs + '\n')
		file.write(histogram + '\n')
		file.close()
	...
	for totalD in [100, 200, 300, 400, 500, 600, 700]:
		histogram, logs = main(totalD, framework=True, discoverBeforeSIB1=True, totalNetworkSlices=totalNetworkSlices, maxSym=maxSym, maxSC=maxSC, blePower=blePower, mnPower=mnPower, getFreqFromGNB=False, printLogs=False)
		file = open('logs/03.log', 'a')
		file.write(logs + '\n')
		file.write(histogram + '\n')
		file.close()
	...
	for totalD in [100, 200, 300, 400, 500, 600, 700]:
		histogram, logs = main(totalD, framework=True, discoverBeforeSIB1=False, totalNetworkSlices=totalNetworkSlices, maxSym=maxSym, maxSC=maxSC, blePower=blePower, mnPower=mnPower, getFreqFromGNB=True, printLogs=False)
		file = open('logs/04.log', 'a')
		file.write(logs + '\n')
		file.write(histogram + '\n')
		file.close()
	...
	for totalD in [100, 200, 300, 400, 500, 600, 700]:
		histogram, logs = main(totalD, framework=True, discoverBeforeSIB1=True, totalNetworkSlices=totalNetworkSlices, maxSym=maxSym, maxSC=maxSC, blePower=blePower, mnPower=mnPower, getFreqFromGNB=True, printLogs=False)
		file = open('logs/05.log', 'a')
		file.write(logs + '\n')
		file.write(histogram + '\n')
		file.close()
	#################################
	...
	for maxS in [5, 10, 14]:
		for maxSc in [1, 2, 3]:
			histogram, logs = main(100, framework=True, discoverBeforeSIB1=True, totalNetworkSlices=totalNetworkSlices, maxSym=maxS, maxSC=maxSc, blePower=blePower, mnPower=mnPower, getFreqFromGNB=True, printLogs=False)
			file = open('logs/06.log', 'a')
			file.write(logs + '\n')
			file.write(histogram + '\n')
			file.close()
	#################################
	...
	# 700 devices, RACH and framework (before SIB1 discovery), 52 network slices
	for maxS in [5, 10, 14]:
		for maxSc in [1, 2, 3]:
			histogram, logs = main(700, framework=True, discoverBeforeSIB1=True, totalNetworkSlices=totalNetworkSlices, maxSym=maxS, maxSC=maxSc, blePower=blePower, mnPower=mnPower, getFreqFromGNB=True, printLogs=False)
			file = open('logs/07.log', 'a')
			file.write(logs + '\n')
			file.write(histogram + '\n')
			file.close()
	#################################